 ...
 * 
 */
package com.iisquare.solr.wltea.core;
...
class AnalyzeContext {
	...
	public AnalyzeContext(String dictSerial, boolean useSmart,
	...
	void initCursor() {
		...
		this.charTypes[this.cursor] = CharacterUtil.identifyCharType(
				this.segmentBuff[this.cursor], useArabic, useEnglish);
	}
	...
	Lexeme getNextLexeme() {
		...
		while (result != null) {
			...
			if (Dictionary.getSingleton(dictSerial).isStopWord(
					this.segmentBuff, result.getBegin(), result.getLength())) {
				// 是停止词继续取列表的下一个
	...
	private void compound(Lexeme result) {
		/*
		 * 统一处理量词 if(!this.useSmart){ return ; }
		 */
		// 数量词合并处理
