...
public class ServletUtil {
	...
	public static Map<String, Object> generateParameterMap(
			Map<String, Object> map, List<String> keyList, String value, int index, int length) {
		int indexNext = index + 1;
		String key = keyList.get(index);
		if (indexNext >= length) { // 当前为最终位置，不存在下级元素
			map.put(key, value); // 默认取最后一个值
			return map;
		}
		...
		map.put(key, generateParameterMap(subMap, keyList, value, indexNext, length));
		return map;
	}
