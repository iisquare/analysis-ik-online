...
import com.iisquare.solr.wltea.dic.Hit;
...
class CJKSegmenter implements ISegmenter {
	...
	CJKSegmenter(String dictSerial) {
		...
		if (CharacterUtil.CHAR_USELESS != context.getCurrentCharType()) {
			...
			if (!this.tmpHits.isEmpty()) {
				...
				for (Hit hit : tmpArray) {
					hit = Dictionary.getSingleton(dictSerial).matchWithHit(
			...
			// *********************************
			// 再对当前指针位置的字符进行单字匹配
			Hit singleCharHit = Dictionary.getSingleton(dictSerial)
					.matchInMainDict(context.getSegmentBuff(),
							context.getCursor(), 1);
